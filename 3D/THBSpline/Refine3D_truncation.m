...
for i=1:size(supp_b,2)%Loop over the support cells of the bspline
    if(supp_b(1,i)~=0 && Elem{level,2}(supp_b(1,i),1)==1 ) %zeros values appending, that's why only consider non-zero
        ...
        nodes1 = Elem{level,9}(supp_b(1,i),:);
        ...
        for k = 1:noci  %Loop over their children cells
            ...
            elseif(k==7)
                ...
                nodes(1,7) = nodes1(1,7);
                Elem{level+1,9}(ind,7) = nodes(1,7);
            end
        end
    end
end
...
for i = 1:size(Btemp,1)
    if(Btemp(1,1)~=0)
        ...
        if(itdu < pU+1)
            ...
            usu = FindSpan(nobfu,pU,knotcu(1,itdu),knotfu)+1;
        end
        ...
        if((nobcu-itdu)<pU)
            ...
            ueu = FindSpan(nobfu,pU,knotcu(1,itdu+pU+1),knotfu)+1;
        end
        ...
        if(itdv < pV+1)
            ...
            usv = FindSpan(nobfv,pV,knotcv(1,itdv),knotfv)+1;
        end
        ...
        if((nobcv-itdv)<pV)
            ...
            uev = FindSpan(nobfv,pV,knotcv(1,itdv+pV+1),knotfv)+1;
        end
        ...
        if(itdw < pW+1)
            ...
            usw = FindSpan(nobfw,pW,knotcw(1,itdw),knotfw)+1;
        end
        ...
        if((nobcw-itdw)<pW)
            ...
            uew = FindSpan(nobfw,pW,knotcw(1,itdw+pW+1),knotfw)+1;
        end
