...
connectNodes = zeros(nelemU(level,1)*nelemV(level,1)*nelemW(level,1),8);
    ...
    for intV = 1:nobV(level,1)
        for intU = 1:nobU(level,1)
            ...
            if((intU>=(pU+1))&&(intV>=(pV+1))&&(intW>=(pW+1)))
                ...
                n1 = ((zind-1)*(nelemV(level,1)+1)*(nelemU(level,1)+1)+(yind-1)*(nelemU(level,1)+1)+xind)+sizem;
                n2 = ((zind-1)*(nelemV(level,1)+1)*(nelemU(level,1)+1)+(yind-1)*(nelemU(level,1)+1)+xind+1)+sizem;
                n3 = ((zind-1)*(nelemV(level,1)+1)*(nelemU(level,1)+1)+(yind)*(nelemU(level,1)+1)+xind+1)+sizem;
                n4 = ((zind-1)*(nelemV(level,1)+1)*(nelemU(level,1)+1)+(yind)*(nelemU(level,1)+1)+xind)+sizem;
                n5 = ((zind)*(nelemV(level,1)+1)*(nelemU(level,1)+1)+(yind-1)*(nelemU(level,1)+1)+xind)+sizem;
                n6 = ((zind)*(nelemV(level,1)+1)*(nelemU(level,1)+1)+(yind-1)*(nelemU(level,1)+1)+xind+1)+sizem;
                n7 = ((zind)*(nelemV(level,1)+1)*(nelemU(level,1)+1)+(yind)*(nelemU(level,1)+1)+xind+1)+sizem;
                n8 = ((zind)*(nelemV(level,1)+1)*(nelemU(level,1)+1)+(yind)*(nelemU(level,1)+1)+xind)+sizem;
                
                connectNodes(E,:) = [n1,n2,n3,n4,n5,n6,n7,n8];
                if(level<=maxlevel-1)
