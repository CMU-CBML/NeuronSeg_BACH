function RHS = compute_Integ_Domain_regularization(Jm,BIGMUXf, BIGMUYf, BIGMUZf, BIGMVXf, BIGMVYf, BIGMVZf, BIGMWXf, BIGMWYf, BIGMWZf,RHSf,PHIU1, PHIV1, PHIW1, lambda_1,lambda_2, lambda_3, w1,w2,w3,H)
%% This function computes the energy functional for regularization in step 2

%% INPUT:
%Jm: matrix of non-zero splines
%BIGMUX, BIGMUY, BIGMUZ: y_xu, y_xv, y_xw
%BIGMVX, BIGMVY, BIGMVZ: y_yu, y_yv, y_yw
%BIGMWX, BIGMWY, BIGMWZ: y_zu, y_zv, y_zw
%RHSf: Right hand side of update of control points in step 1
%PHIU1: Derivative w.r.t u Basis function matrix
%PHIV1: Derivative w.r.t v Basis function matrix
%PHIW1: Derivative w.r.t w Basis function matrix
%lambda1, lambda2, lambda3: parameters for length, area and volume term
    ...
    for gg1 = 1:xlen
        for gg2 = 1:xlen
            for gg3 = 1:xlen
                ...
                reg_length_x(:,gg1,gg2,gg3) = 2.*lambda_1.*(term_fxx(gg1,gg2,gg3)).*phi_ui+term_fxy(gg1,gg2,gg3).*phi_vi+term_fxz(gg1,gg2,gg3).*phi_wi;
                reg_length_y(:,gg1,gg2,gg3) = 2.*lambda_1.*term_fyx(gg1,gg2,gg3).*phi_ui+(term_fyy(gg1,gg2,gg3)).*phi_vi+term_fyz(gg1,gg2,gg3).*phi_wi;
                reg_length_z(:,gg1,gg2,gg3) = 2.*lambda_1.*term_fzx(gg1,gg2,gg3).*phi_ui+term_fzy(gg1,gg2,gg3).*phi_vi+(term_fzz(gg1,gg2,gg3)).*phi_wi;
