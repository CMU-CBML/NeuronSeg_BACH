function [M,phi,VF] = MultipleResolution3D_heliximg(M,F,phi,param,Img1)
...
for level = param.maxlevel:-1:1
    ...
    M = resize3Dmatrix(size(M00,1), size(M00,2), size(M00,3),M);
    F = resize3Dmatrix(size(M00,1), size(M00,2), size(M00,3),F);
    
    phi = resize3Dmatrix(size(M00,1), size(M00,2), size(M00,3),phi);
