...
%This function computes the basis function at different evaluation points
...
%nobU,nobV: number of basis functions in u and v directions
%nelemU: number of elements in u direction

%--Output Variable:
%Jm: Struct variable storing support B-splines over each active cell at the
...
%HH: element size of each active cell
%PHI,PHIU,PHIV: Variables storing the basis function and its derivatives at
%each Gaussian point (phi, phi_x, phi_y)
%BIGX,BIGY: The Gaussian point coordinates in each active cell
...
for i = 1:ac_ct
    ...
    [s1,~] = ggquad(orderGauss);
    [s2,~] = ggquad(orderGauss);
    xlen = size(s1,1);
