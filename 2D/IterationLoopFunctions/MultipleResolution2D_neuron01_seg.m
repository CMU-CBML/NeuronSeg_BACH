function [M,phi,VF] = MultipleResolution2D_neuron01_seg(M,F,phi,param)
...
for level = param.maxlevel:-1:1
    ...
    CIF = coef_F(:)';
    coef_matF = reshape(CIF, npy+param.pU, npx+param.pV); % this is an unnecessary step, check later!!!!
    
    M = imresize(M,scale);
    vecM = M(:)';
    coef_M= img2coef2D(npx,npy, vecM);
    CIM = coef_M(:)';
    coef_matM = reshape(CIM, npy+param.pU, npx+param.pV);
    ...
    CIMorig = coef_Morig(:)';
    coef_matMorig = reshape(CIMorig, npy+param.pU, npx+param.pV);
    ...
    if(level==param.maxlevel)
        ...
        CIM = coef_M(:)';
        coef_matM = reshape(CIM, ny+param.pU, nx+param.pV);
    ...
    for multilev = 0:1:maxlev-1
        ...
        [Jm,Coeff,Pixel,HH,PHI,PHIU,PHIV,BIGX,BIGY] = constructAdaptiveGrid(ac,param,Dm,Em,M,knotvectorU,knotvectorV,multilev,nobU,nobV,nelemU);
    ...
    term_reg = zeros(ac_ct*orderGauss,orderGauss,2);
    term_seg = zeros(ac_ct*orderGauss,orderGauss,3);
    
    %param.sigma =10;% the key parameter which needs to be tuned properly.
    %param.sigma_phi = 50;% the variance of regularized Gaussian kernel
